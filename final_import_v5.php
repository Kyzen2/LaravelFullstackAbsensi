...
// Create a new Guru for this class to avoid Unique constraint on wali_kelas_id
$guruUser = User::firstOrCreate(['serial_number' => 'GURU_PPLG'], [
    'name' => 'Wali Kelas PPLG',
    'password' => Hash::make('password'),
    'role' => 'guru',
]);

$guru = Guru::firstOrCreate(['user_id' => $guruUser->id], [
    'nama_guru' => 'Wali Kelas PPLG',
    'nip' => 'GURU_PPLG',
]);

// Use updateOrCreate for Kelas instead of firstOrCreate to handle the unique constraint better if it already exists partially
...
foreach ($students as $student) {
    try {
        $u = User::updateOrCreate(['serial_number' => $student['nisn']], [
            'name' => $student['nama'],
            'password' => Hash::make('password'),
            'role' => 'siswa',
        ]);
        
        $s = Siswa::updateOrCreate(['user_id' => $u->id], [
            'nisn' => $student['nisn'],
            'nama_siswa' => $student['nama'],
        ]);
        
        AnggotaKelas::updateOrCreate(['siswa_id' => $s->id, 'kelas_id' => $kelas->id]);
        $importCount++;
    } catch (\Exception $e) {
        echo "ERROR IMPORTING " . $student['nama'] . ": " . $e->getMessage() . "\n";
    }
}

$out = "TOTAL IMPORTED: " . $importCount . "\n";
$out .= "VERIFYING DATABASE COUNT: " . User::where('role', 'siswa')->count() . "\n";

$turky = User::where('serial_number', '0081506973')->first();
if ($turky) {
    $out .= "TURKY VERIFIED IN DB!\n";
} else {
    $out .= "TURKY STILL MISSING!\n";
}

file_put_contents('final_import_v5_result.txt', $out);
echo $out;
