...
foreach ($students as $student) {
    User::updateOrCreate(['serial_number' => $student['nisn']], [
        'name' => $student['nama'],
        'password' => Hash::make('password'),
        'role' => 'siswa',
    ]);
    $u = User::where('serial_number', $student['nisn'])->first();
    Siswa::updateOrCreate(['user_id' => $u->id], [
        'nisn' => $student['nisn'],
        'nama_siswa' => $student['nama'],
    ]);
    $s = Siswa::where('user_id' ,$u->id)->first();
    AnggotaKelas::updateOrCreate(['siswa_id' => $s->id, 'kelas_id' => $kelas->id]);
}

dd("DONE. COUNT IS: " . Siswa::count());
